 ...
 */
@Service
public class GPSSetService {
	private final Log logger = LogFactory.getLog(GPSSetService.class);
...
//	private MsgSendUtil msgSendUtil;
	 ...
	 *  设置OBD的GSP状态
	 * @param obdSn 设备Sn
	 * @param state 00 开启 01关闭
	 * @return 00成功 01失败 其他保留
	 */
	public String GSPSet(String obdSn,String state){
		MsgSendUtil msgSendUtil = new MsgSendUtil();
		Dictionary dic=dictionaryService.getDicByCodeAndType("owner.command", "server.GSP");
		...
		logger.info(resultStr+"*************************801d客户端响应结果");
		if(resultStr!=null){
