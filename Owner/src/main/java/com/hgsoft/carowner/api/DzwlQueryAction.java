...
@Controller
@Scope("prototype")
public class DzwlQueryAction extends BaseAction {
	...
	public void entryInterface() {
		...
		try {
			...
			if (jso.optString("checkCode").equals(CHECK_SUCCESS)) {
				...
				returnJson = this.dzwlQuery(recJson);
			} else {
	 ...
	 * 
	 */
	private JSONObject dzwlQuery(JSONObject jso) throws Exception {

		String deviceId = jso.optString("deviceId");// 待绑定设备厂商的唯一标识。原设备厂商的设备序列号
		obdApiLogger.info("----------【电子围栏查询】---" + deviceId);
		...
		try {
			...
			if (obdStockInfo == null) {
				obdApiLogger.info("----------【电子围栏查询】---" + deviceId + "---该设备未注册,请联系管理员.");
				desc = "设备不存在,请联系管理员.";
				return returnJson(-1, -1, desc);
			}
			
			String obdSn = obdStockInfo.getObdSn();
			List<Integer> validSet = new ArrayList<Integer>();
			validSet.add(1);
			validSet.add(2);
			//查询离线设置
			List<Fence> fences=fenceService.queryList(obdSn, null, validSet);
			if(fences!=null && fences.size()>0){
				JSONObject json = new JSONObject();
				for (Fence fence : fences) {
					Integer type = fence.getType();
					Integer alert = fence.getAlert();
					Integer areaNum = fence.getAreaNum();
					...
					Date createTime = fence.getCreateTime();
					Date updateTime = fence.getUpdateTime();
					Integer valid = fence.getValid();
					json.put("type", type);
					json.put("alert", alert);
					json.put("areaNum", areaNum);
					json.put("points", points);
					json.put("timerType", timerType);
					switch (timerType) {
					case 1:
						json.put("dayWeek", dayWeek);
						break;
					case 3:
				    	String startDateStr = yMd.format(startDate);
				    	String endDateStr = yMd.format(endDate);
			    	...
			    	String startTimeStr = Hms.format(startTime);
			    	String endTimeStr = Hms.format(endTime);
			    	json.put("startTime", startTimeStr);
					json.put("endTime", endTimeStr);		
					String createTimeStr = yMdHms.format(createTime);
					json.put("createTime", createTimeStr);	
					if(updateTime!=null){
						String updateTimeStr = yMdHms.format(updateTime);
						json.put("updateTime", updateTimeStr);	
					}
		...
		// 当前状态：0成功 -1失败
		resultJso.put("efenceData", jArray);
		resultJso.put("state", state);
		retJso.put("result", resultJso);
		obdApiLogger.info(deviceId + "-----------电子围栏查询结果:" + retJso);
		return retJso;
	}
