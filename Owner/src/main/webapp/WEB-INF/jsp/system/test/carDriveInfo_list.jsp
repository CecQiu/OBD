...
<body>
	<form name="myForm" id="myForm" action="carDriveInfo_query.do" method="post" onsubmit="return formCheck();">
		<ul class="breadcrumb">
		...
		<div class="widget widget-table">
			...
			<div class="widget-content widget-list" style="overflow-x: scroll;">
				<table class="table table-striped table-bordered table-condensed table-hover sortable">
					...
					<tbody>
						<c:forEach items="${carDriveInfoList}" var="item" varStatus="status">
							<tr>
								<td>${item.driveInfoId}</td>
								<td>${item.carId}</td>
								<td>${item.obdSn}</td>
								<td>${item.tmsSpeeding}</td>
								<td>${item.tmsFatigue}</td>
								<td>${item.tmsRapDec}</td>
								
								<td>${item.tmsRapAcc}</td>
								<td>${item.highSpeed}</td>
								<td>${item.tmsSharpTurn}</td>
								<td>${item.notMatch}</td>
								<td>${item.longLowSpeed}</td>
								<td>${item.tmsBrakes}</td>
								
								<td>${item.tmsSteep}</td>
								<td>${item.avSpeed}</td>
								
								<td><fmt:formatDate value="${item.driveDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td>
									<c:choose>
