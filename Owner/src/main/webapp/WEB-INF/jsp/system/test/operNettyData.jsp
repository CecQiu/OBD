...
function query(type){
	...
	var hrefStr = '${basePath}/testOBD/redisDataControlAction.do';
...
<body>
<form id="myForm" class="myForm" action="${basePath}/admin/nettyDataControlAction.do" method="post">
	<ul class="breadcrumb">
		<li><i class="icon-home icon-2x"></i></li>
		<li>当前位置：监控管理  &gt; 操作netty据</li>
	</ul>
	<h3><c:if test="${shopName != null && shopName != ''}">欢迎${shopName }</c:if></h3>
	<div class="widget">
		<div class="widget-content" style="line-height: 20px">
			Netty状态:<br />
			bossState:${bossState }<br />
			workerState:${workerState }<br />
			
			<button <c:if test="${bossState == '开启' }">disabled="disabled"</c:if> 
				type="submit" class="btn btn-success" value="1" name="switchState">打开</button>
			
			<button <c:if test="${bossState == '关闭' }">disabled="disabled"</c:if>
				type="submit" class="btn btn-danger" value="0" name="switchState">关闭</button>
