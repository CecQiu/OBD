...
<script language="javascript">
	...
	function formCheck(){
		...
		if(obdSn==''&& obdMSn=='' && startTime=='' && endTime ==''){
			alert("请输入查询参数.");
			return false;
		}
	}
...
<body>
	<form name="myForm" id="myForm" action="obdMiles_query.do" method="post" onsubmit="return formCheck();">
		<ul class="breadcrumb">
		...
		<div class="widget widget-table">
			...
			<div class="widget-content widget-list" style="overflow-x: scroll;">
				<table class="table table-striped table-bordered table-condensed table-hover sortable">
					...
					<tbody>
						<c:forEach items="${obdMilesList}" var="item" varStatus="status">
							<tr>
								<td>${item.id}</td>
								<td>${item.obdSn}</td>
								<td>${item.miles}</td>
