...
<body>
	...
	<div style="float: left;width: 165px;height: 100%;overflow:scroll;">
		<ol id="userList" class="list-unstyled" style="margin-left: 4px;margin-top: 10px;font-size: 20px">
			<li><b>用户列表</b></li>
			
				<%-- <li style="background: white;"><input value="${item.obdSN}" onchange="remarkPosition(this,'${item.name}','${item.license}','${item.mobileNumber}');" type="checkbox"/>${item.license}</li> --%>
				<li style="background: white;"><input value="${mebUser.obdSN}" type="radio" name="user" userName="${mebUser.name}" license="${mebUser.license}" mobileNumber="${mebUser.mobileNumber}"
					onClick="remarkPosition(this);"/>${mebUser.obdSN}</li>
		</ol>
		双击获取的baidu坐标：
		...
		是否查看轨迹时间：<br/>
		<input type="checkbox" id="islook" onclick="isLook();"/>
		间隔几个点？<input type="text" id="count" value='2' size="1"/>&nbsp;&nbsp;<br/><br/>
	...
	<script type="text/javascript">
		...
		function remarkPosition(val){
			var name = jQuery(val).attr('userName');
			var plate = jQuery(val).attr('license');
			var phone = jQuery(val).attr('mobileNumber');
			//map.clearOverlays();
			...
			var sn = jQuery(val).val();
			if(sn != null) {
				jQuery.ajax({
					url:'carPosition_getPosition.do',
					data:{'sn':sn},
					type:"POST",
				    dataType:"json",
					success:function(data) {
						if(data.status == 'success' && data.message.longitude != '' && data.message.longitude != undefined) {
							var d = new Date();
							d.setTime(data.message.time.time);
							po_msg = '最后定位时间：'+d.format("yyyy-MM-dd HH:mm:ss");
							
							var point = new BMap.Point(data.message.longitude,data.message.latitude);
							/* BMap.Convertor.translate(point,0,translateCallback); */
							//alert(data.message.longitude);alert(data.message.latitude);
							...
							var sContent = '<div><table style="text-align:center;margin-top:10px;margin-bottom:0px" class="table"><tr><th>车主姓名：</th><td>'
				        		+ name + '</td><th>车牌号：</th><td>' + plate + '</td></tr><tr><th>手机号：</th><td colspan="3">'
				        		+ phone + '</td></tr><tr><th>位置信息：<th><td colspan="3" align="center">';;
				        	labels.push(sContent);
				        	labels.push('</td></tr><tr><td colspan="4"><a onclick="playTack(\''
					        		+ sn + '\',\''+name+'\',\''+plate+'\');" class="btn">查询轨迹</a></td></tr></table></div>');
						} else if(data.status == 'success'){
							//var point = new BMap.Point('113.316608','23.140437');
							//changePosition(point);
							alert(plate + " 暂时没有位置信息");
						} else {
							alert(data.message);
						}
						...
						alert("请求错误,可能会话超时,定时查询最新位置暂停,请刷新页面.");
					}
				});
			}
		}
