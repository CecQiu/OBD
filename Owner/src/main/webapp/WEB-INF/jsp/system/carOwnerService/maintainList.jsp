...
function add(){
	var url="carOwnerService_maintainAddJsp.do";
...
function del(){
	...
	url="carOwnerService_maintainDel.do?idArr="+infoId;
...
function edit(){
	...
	if(length!=1){
		...
		var url="carOwnerService_maintainEditJsp.do?id="+value;
		document.getElementById("editUser").style.display="";
		document.getElementById("iframe").src=url;
	}
...
<body>
<form id="myForm" class="myForm" action="carOwnerService_maintainSearch.do" method="post">
	<ul class="breadcrumb">
		<li><i class="icon-home icon-2x"></i></li>
		<li>当前位置：车主服务  &gt; 保养提醒</li>
	</ul>
	...
	<div class="widget">
		<div class="widget-content" style="line-height: 20px">
			...
			保养状态：<select name="maintainStatus">
				<option id="maintainStatus" value="">全部</option>
			...
			订单状态：<select name="orderStatus">
				<option id="orderStatus" value="">全部</option>
		...
		<div class="widget-content">
			<table class="table table-hover">
			  <tr style="background-color: #c4e3f3;">
			  	<td>选择</td><td>姓名</td><td>性别</td><td>手机号码</td><td>车牌号码</td><td>二维码</td><td>保养类别</td><td>上次保养时间</td>
			  </tr>
			  <s:iterator value="carMaintainList" >
			  <tr>
				...
				<td>
			  	...
			  	<td>
			  	  	<c:if test="${maintainStatus=='00'}"><span>无需保养</span></c:if>
			  	  	<c:if test="${maintainStatus=='01'}"><span>需要保养</span></c:if>
			  	  	<c:if test="${maintainStatus=='02'}"><span>逾期保养</span></c:if>
			  	</td>
			  	<td>
			  	<s:property value="maintainTime"></s:property>
			  	  	<%-- <fmt:formatDate value="${maintainTime}" type="both" /> --%>
			  	</td>
			  </tr> 
			  </s:iterator>
			</table>
